...
# PASO 1: Especificar composicion PRIMERO (OBLIGATORIO)
# HYSYS necesita saber que componentes estan presentes antes de T/P/F
# ComponentMolarFraction.Values devuelve TUPLA (inmutable)
comp_frac_tuple = stream1.ComponentMolarFraction.Values
comp_frac = list(comp_frac_tuple)  # Convertir a lista
comp_frac[0] = 1.0  # 100% Methanol (indice 0)
stream1.ComponentMolarFraction.Values = comp_frac  # Reasignar

# PASO 2: Especificar condiciones termodinamicas (DESPUES de composicion)
stream1.TemperatureValue = 25 + 273.15  # K
stream1.PressureValue = 101.325          # kPa
stream1.MolarFlowValue = 100.0           # kgmole/h

# PASO 3: Leer propiedades calculadas (requiere GetValue())
...
\begin{enumerate}
    \item NO usar \texttt{ComponentMolarFractionValue("Methanol", 1.0)}
    \\Causa error: \texttt{'tuple' object is not callable}
    \item NO establecer T/P/F ANTES de la composicion
    \\Causa error COM -2147352567: \texttt{'Ocurrio una excepcion'}
    \item NO modificar directamente la tupla: \texttt{comp\_frac[0] = 1.0}
    \\Causa error: \texttt{'tuple' object does not support item assignment}
    \\Solucion: Convertir a lista primero con \texttt{list()}
\end{enumerate}
\end{warningbox}
...
\textbf{Hallazgos:}
\begin{itemize}
    \item \textbf{ORDEN CRITICO:} Composicion primero, luego T/P/F
    \item \textbf{Composicion:} \texttt{ComponentMolarFraction.Values} devuelve TUPLA
    \item \textbf{Modificacion:} Convertir tupla a lista con \texttt{list()}, modificar, reasignar
...
-2147352567 & Ocurrió una excepción & Nombre de componente inválido, propiedad no definida, orden incorrecto en corrientes & Usar "H2O" en lugar de "Water", establecer composición ANTES de T/P/F \\
-2147024891 & Acceso denegado & Establecer T/P/F antes de composición en corrientes & Establecer ComponentMolarFraction.Values primero \\
...
def main():
    try:
        ...
        # IMPORTANTE: Composicion PRIMERO, antes de T/P/F
        # Values devuelve tupla, convertir a lista
        comp_frac = list(stream1.ComponentMolarFraction.Values)
        comp_frac[0] = 1.0  # 100% Methanol
        stream1.ComponentMolarFraction.Values = comp_frac

        # Ahora si, condiciones termodinamicas
